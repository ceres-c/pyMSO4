...
The software architecture of the oscilloscope seems to be more reminiscent of a monolithic, tightly integrated embedded system rather than a modular system. No SDK is provided for this specific device by the vendor, and the only way to control the oscilloscope is through the graphical UI or \gls{visa}. A quick reverse engineering of the firmware showed that the UI is controlling directly the analog frontend of the device, and similarly the \gls{lxi} server running on the device goes through the UI binary to control the device.

This becomes problematic when the \gls{lxi} server crashes and stops responding, causing the UI to also become unresponsive, thus requiring manual interaction to reboot the oscilloscope. Crashes are sufficiently frequent to make this an issue. Crashes will happen in multiple circumstances, but they can mostly be summed up in two categories:
\begin{itemize}[nosep]
...
\subsection{Wiring}
The oscilloscope is connected to the host computer via both Ethernet and USB. Ethernet is used to control the instrument as well as acquiring data, while USB is used as a backup control interface if the Ethernet connection is lost. This is required because, as stated in \autoref{sec:fw-info}, the visa implementation on the \gls{vr} is not reliable and often crashes, requiring a reset of the oscilloscope.
\begin{itemize}
	\item \textbf{Ethernet}: The oscilloscope is connected to a router, to which the host computer is also connected. The router provides a DHCP server to assign an IP address to the oscilloscope. % TODO check if p2p ethernet works
	\begin{scope_setting}
		Utility\ \rightarrow\  I/O\ldots\ \rightarrow\  LAN\newline
		DHCP: ON\newline % TODO check this
		LAN Status: OK
	\end{scope_setting}
	...
	\begin{scope_setting}
		...
		USB Device Port: ON % TODO check this
	\end{scope_setting}
\end{itemize}
...
\begin{minted}[frame=lines,breaklines,autogobble]{bash}
	...
	list
\end{minted}
And there should be at least 2 entries in the output, one for the USB device and one for the Ethernet device
